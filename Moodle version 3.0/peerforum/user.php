...
foreach ($result->posts as $post) {
    ...
    $postoutput[] = peerforum_print_post($post, $discussion, $peerforum, $cm, $course, false, false, false, $fulllink, '', null, true, null, true, false, false, true, null);
...
foreach ($result->posts as $post) {
    ...
    // load peergrades
    if ($peerforum->assessed != PEERGRADE_AGGREGATE_NONE) {
        $peergradeoptions->context = $cm->context;
        $peergradeoptions->items = array($post);
        ...
        $peergradeoptions->peergradeassesstimestart = $peerforum->peergradeassesstimestart;
        $peergradeoptions->peergradeassesstimefinish = $peerforum->peergradeassesstimefinish;
        if ($peerforum->type == 'single' or !$post->discussion) {
            $peergradeoptions->returnurl = $peerforumurl;
        } else {
            $peergradeoptions->returnurl = $discussionurl;
        }

        $updatedpost = $pm->get_peergrades($peergradeoptions);
        //updating the array this way because we're itepeergrade over a collection and updating them one by one
        $result->posts[$updatedpost[0]->id] = $updatedpost[0];
    }
