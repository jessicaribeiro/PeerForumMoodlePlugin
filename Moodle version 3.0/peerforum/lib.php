...
function get_student_status($userid, $courseid){
    ...
    $status = $DB->get_record('peerforum_peergrade_users', array('courseid'=>$courseid, 'iduser' => $userid))->userblocked;
...
function get_time_expire($postid){
    global $CFG, $DB;
    //when the post was created
    $time_created_db = usergetdate(time_created($postid));
...
function get_all_posts_info(){
    ...
    foreach($posts as $postid => $values){
        if (user_has_role_assignment($posts[$postid]->userid, 5)) {
            ...
            $info_post->timeexpire = get_time_expire($postid)->d;
...
function get_students_assigned($courseid, $postid){
    ...
    foreach ($peers as $i => $value) {
        ...
        $posts = $DB->get_record('peerforum_peergrade_users', array('courseid'=>$courseid, 'iduser' => $id));
        if(!empty($posts)){
            $posts_done = $posts->postspeergradedone;
...
///////
    ...
    if (!empty($post->peergrade)) {
        ...
        //$output .= html_writer::tag('div', $OUTPUT->render($post->peergrade), array('class'=>'peerforum-post-peergrade'));
    }
