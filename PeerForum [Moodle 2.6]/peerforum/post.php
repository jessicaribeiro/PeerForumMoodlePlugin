    ...
    if (!empty($confirm) && confirm_sesskey()) {    // User has confirmed the delete
        ...
        if ($replycount) {
            ...
            echo $OUTPUT->confirm(get_string("deletesureplural", "peerforum", $replycount+1),
                         ...
                         $CFG->wwwroot.'/mod/peerforum/discuss.php?d='.$post->discussion.'&page='.$currentpage.'#p'.$post->id);
...
if ($fromform = $mform_post->get_data()) {
    ...
    if ($fromform->edit) {           // Updating a post
        ...
        if ($fromform->id = peerforum_add_new_post($addpost, $mform_post, $message)) {
            ...
            redirect(peerforum_go_back_to("$discussionurl&page=$currentpage#p$fromform->id"), $message.$subscribemessage, $timemessage);
