...
function peerforum_print_post($post, $discussion, $peerforum, &$cm, $course, $ownpost=false, $reply=false, $link=false,
    ...
    if ($peerforum->type == 'single' and $discussion->firstpost == $post->id) {
        ...
        $commands[] = array('url'=>new moodle_url('/mod/peerforum/post.php', array('edit'=>$post->id, 'page' => $currentpage)), 'text'=>$str->edit);
    }

    if ($cm->cache->caps['mod/peerforum:splitdiscussions'] && $post->parent && $peerforum->type != 'single') {
        $commands[] = array('url'=>new moodle_url('/mod/peerforum/post.php', array('prune'=>$post->id, 'page' => $currentpage)), 'text'=>$str->prune, 'title'=>$str->pruneheading);
    }
    ...
    if ($peerforum->type == 'single' and $discussion->firstpost == $post->id) {
        ...
        $commands[] = array('url'=>new moodle_url('/mod/peerforum/post.php', array('delete'=>$post->id, 'page' => $currentpage)), 'text'=>$str->delete);
    }
