     ...
     if(!empty($array_users)){
         ...
         if(!empty($groups)){
             foreach ($groups as $id => $value) {
                 ...
                 if(in_array($postauthor, $students)){
                     $user_group = $groups[$id]->groupid;
                 }
                 if(in_array($peer, $students)){
                     $peer_group = $groups[$id]->groupid;
                 }

                 if($user_group != null && $peer_group != null){
                     break;
                 }
             }
         ...
         if($conflit == 0){
             if($user_group != $peer_group){
                 ...
                 else if(!in_array($peer, $array_peergraders) && !(in_array($peer, $peers))){
                     array_push($array_peergraders, $peer);
                 }
...
class peergrade implements renderable {
     ...
     *             This allows a plugin to rate more than one thing by specifying different peergrade areas
     */
...
public function verify_post_expired($postid, $peerforum, $peergrade){
    ...
    else if($time_current <= $time_finish){
        $data->post_expired = false;
        $data->time_interval = $time_interval;
        $data->time_current = $time_current;
...
public function time_to_see_feedbacks($peerforum, $userid, $postid){
    global $DB, $PAGE;
    ...
    if(!has_capability('mod/peerforum:viewallpeergrades', $PAGE->context)){
        if($peerforum->whenfeedback == 'always'){
            ...
            if(!empty($peergrade)){
                $peergradeauthor = $peergrade->userid;
            } else {
                $peergradeauthor = -1;
            }

            if($peergradeauthor == $userid){
                return 1;
            } else {
...
public function time_to_see_grades($peerforum, $userid, $postid){
    global $DB, $PAGE;
...
public function can_see_grades($peerforum, $userid, $postid){
    global $DB, $PAGE;

    if(!has_capability('mod/peerforum:viewallpeergrades', $PAGE->context)){
        if($peerforum->peergradesvisibility == 'onlyprofessor'){
            $peergradeauthor = $DB->get_record('peerforum_peergrade', array('itemid' => $postid))->userid;
            ...
            if($peerforum->peergradesvisibility == 'public'){
                ...
                else if($peergradeauthor == $userid){
                    return 1;
                }
                else {
                    return 0;
...
public function can_see_feedbacks($peerforum, $userid, $postid){
    global $DB, $PAGE;
...
class peergrade_manager {
    ...
    public function delete_post_peergrade($postid, $courseid){
        ...
        $sql = "SELECT p.iduser, p.id, p.postspeergradedone, p.poststopeergrade, p.postsblocked
                  FROM {peerforum_peergrade_users} p
    ...
    public function get_post_peergrader($id) {
        ...
        public function update_peergrader_posts($userid, $postid, $courseid){
            ...
            if(empty($posts) && has_capability('mod/peerforum:viewallpeergrades', $PAGE->context)){
                $peerforumid = $DB->get_record('course_modules', array('id' => $PAGE->context->instanceid))->instance;
     ...
     *
     * @param stdClass $options {
     *      userid => int the id of the user whose items were rated, NOT the user who submitted peergrades. 0 to update all. [required]
    ...
    /**
     * Looks for a callback like forum_peergrade_permissions() to retrieve permissions from the plugin whose items are being rated
     *
