...
function peerforum_add_instance($peerforum, $mform = null) {
    ...
    if (empty($peerforum->courseid)) {
        $peerforum->courseid = $peerforum->course;
    }

    $peerforum->id = $DB->insert_record('peerforum', $peerforum);

    // we need to use context now, so we need to make sure all needed info is already in db
    $cmid = $peerforum->coursemodule;
    $DB->set_field('course_modules', 'instance', $peerforum->id, array('id' => $cmid));
    ...
    if ($peerforum->type == 'single') {  // Create related discussion.
        ...
        $discussion->peergradeassessed  = $peerforum->peergradeassessed;
...
function assign_peergraders($user, $postid, $courseid) {
    ...
    if($isstudent){
        ...
        $peers = randomGen($array_peers, $max_peers, $max_peers);
...
function peerforum_update_instance($peerforum, $mform) {
    ...
    if(empty($peerforum->finalgrademode)){
        $peerforum->finalgrademode = $DB->get_record('peerforum', array('id'=>$peerforum->id))->finalgrademode;
    }
    ...
    if (($oldpeerforum->assessed<>$peerforum->assessed) or ($oldpeerforum->scale<>$peerforum->scale) or ($oldpeerforum->peergradescale<>$peerforum->peergradescale) or ($oldpeerforum->peergradeassessed<>$peerforum->peergradeassessed)) {
    ...
    if ($posts = peerforum_get_unmailed_posts($starttime, $endtime, $timenow)) {
        ...
        foreach ($posts as $pid => $post) {
            ...
            if (!isset($subscribedusers[$peerforumid])) {
                ...
                if ($subusers = \mod_peerforum\subscriptions::fetch_subscribed_users($peerforums[$peerforumid], 0, $modcontext, 'u.*', true)) {
    ...
    if ($users && $posts) {
        ...
        foreach ($users as $userto) {
            ...
            foreach ($posts as $pid => $post) {
                ...
                if (!\mod_peerforum\subscriptions::is_subscribed($userto->id, $peerforum, $post->discussion, $coursemodules[$peerforum->id])) {
...
function get_post_status($postid, $userid, $courseid){
    ...
    if(!empty($posts_user)){
        ...
        if(!empty($posts_topeergrade)){
            ...
            if(in_array(-1, $posts_topeergrade)){
                $a = array_search(-1, $posts_topeergrade);
                unset($posts_topeergrade[$a]);
                $posts = implode(';', $posts_topeergrade);
                $data = new stdClass();
                $data->id = $posts_user->id;
...
function get_time_expire($postid, $userid){
    ...
    $time_assigned_db = usergetdate(time_assigned($postid, $userid));
...
function peerforum_update_grades($peerforum, $userid=0, $nullifnone=true) {
    ...
    if (!$peerforum->peergradeassessed || !$peerforum->assessed) {
        ...
        $rategrades = peerforum_get_user_grades($peerforum, $userid);
    ...
    if (!empty($rategrades && $peerforum->assessed)) {
       peerforum_grade_item_update($peerforum, null, null, $rategrades);
    }
         ...
         if(empty($peergradesprofessors) && empty($peergradesstudents) && empty($rategrades)){
             peerforum_grade_item_update($peerforum, $grade, $grade, $grade);
         }
         else if(empty($rategrades) && !empty($peergradesprofessors) && !empty($peergradesstudents)){
            peerforum_grade_item_update($peerforum, $peergradesprofessors, $peergradesstudents, $grade);
         }
         else if(empty($rategrades) && empty($peergradesprofessors) && !empty($peergradesstudents)){
             peerforum_grade_item_update($peerforum, $grade, $peergradesstudents, $grade);
         }
         else if(empty($rategrades) && !empty($peergradesprofessors) && empty($peergradesstudents)){
             peerforum_grade_item_update($peerforum, $grade, $grade, $peergradesstudents);
         }
////////////
         else if(empty($peergradesstudents) && !empty($peergradesprofessors) && !empty($rategrades)){
            peerforum_grade_item_update($peerforum, $peergradesprofessors, $grade, $rategrades);
         }
         else if(empty($peergradesstudents) && empty($peergradesprofessors) && !empty($rategrades)){
            peerforum_grade_item_update($peerforum, $grade, $grade, $rategrades);
         }
         else if(empty($peergradesstudents) && !empty($peergradesprofessors) && empty($rategrades)){
            peerforum_grade_item_update($peerforum, $peergradesprofessors, $grade, $rategrades);
         }
//////////
         else if(empty($peergradesprofessors)&& !empty($peergradesstudents) && !empty($rategrades)){
            peerforum_grade_item_update($peerforum, $grade, $peergradesstudents, $rategrades);
         }
         else if(empty($peergradesprofessors) && empty($peergradesstudents) && !empty($rategrades)){
            peerforum_grade_item_update($peerforum, $grade, $grade, $rategrades);
         }
...
function peerforum_grade_item_update($peerforum,  $peergradesprofessors=NULL, $peergradesstudents=NULL, $rategrades=NULL) {
    ...
    if($peerforum->peergradeassessed){
        ...
        $item = array();
        $item['idnumber'] = $peerforum->cmidnumber;
        $item['itemname'] = get_string('gradeitemstudentpeergrade', 'peerforum', $a);
        ...
        if (!$peerforum->peergradeassessed || $peerforum->peergradescale == 0) {
           ...
           $item['gradetype'] = GRADE_TYPE_VALUE;
           $item['grademax']  = $peerforum->studentpercentage;
           $item['grademin']  = 0;
       }
        else if ($peerforum->peergradescale < 0) {
            $item['gradetype'] = GRADE_TYPE_SCALE;
        ...
        if($peerforum->peergradeassessed){
            grade_update('mod/peerforum', $peerforum->course, 'mod', 'peerforum', $peerforum->id, 0, $peergradesstudents, $item);
        }

        $item = array();
        $item['idnumber'] = $peerforum->cmidnumber;
        $item['itemname'] = get_string('gradeitemrate', 'peerforum', $a);
        ...
        if ($rategrades  === 'reset') {
            $rategrades['reset'] = true;
            $rategrades = NULL;
        }
        if($peerforum->assessed){
            grade_update('mod/peerforum', $peerforum->course, 'mod', 'peerforum', $peerforum->id, 2, $rategrades, $item);
        }
}
    ...
    foreach ($peers as $i => $value) {
        ...
        if(!empty($posts_done_db)){
            ...
            if(in_array(-1, $posts)){
                $a = array_search(-1, $posts);
                unset($posts[$a]);
                $posts_new = implode(';', $posts);
                $data = new stdClass();
                $data->id = $posts_done_db->id;
...
function get_students_can_be_assigned($courseid, $postid, $postauthor){
        ...
        foreach ($students as $id => $value) {
            ...
            if(!in_array($student, $peergraders)){
                ...
                if(!empty($peergraders_info)){
                    ...
                    if(in_array(-1, $posts_tograde)){
                        $a = array_search(-1, $posts_tograde);
                        unset($posts_tograde[$a]);
                        $posts_new = implode(';', $posts_tograde);
                        $data = new stdClass();
                        $data->id = $peergraders_info->id;
                        $data->poststopeergrade = $posts_new;
                        $DB->update_record('peerforum_peergrade_users', $data);
                    }

                    if(in_array(-1, $block)){
                        $a = array_search(-1, $block);
                        unset($block[$a]);
                        $posts_new = implode(';', $block);
                        $data = new stdClass();
                        $data->id = $peergraders_info->id;
                    ...
                    if(in_array(-1, $posts_graded)){
                        $a = array_search(-1, $posts_graded);
                        unset($posts_graded[$a]);
                        $posts_new = implode(';', $posts_graded);
                        $data = new stdClass();
                        $data->id = $peergraders_info->id;
                        $data->postspeergradedone = $posts_new;
                        $DB->update_record('peerforum_peergrade_users', $data);
    ...
    if(in_array($postauthor, $can_be_assigned)){
        $key = array_search($postauthor, $can_be_assigned);
        unset($can_be_assigned[$key]);
    }

    //students cannot be from the same group
    /*$groups = $DB->get_records('peerforum_groups', array('courseid' => $courseid));

    if(!empty($groups)){
        foreach ($groups as $k => $value) {
        $students = explode(';', $groups[$k]->studentsid);
        $students = array_filter($students);

        foreach ($can_be_assigned as $i => $value) {
            //verify if they are from the same group
            if(in_array($can_be_assigned[$i], $students) && in_array($postauthor, $students)){
                $key = array_search($can_be_assigned[$i], $can_be_assigned);
                unset($can_be_assigned[$key]);
    ...
    foreach ($conflicts as $key => $value) {
        ...
        if(in_array(-1, $conflictstds)){
            $a = array_search(-1, $conflictstds);
            unset($conflictstds[$a]);
            $students = implode(';', $conflictstds);
            $data = new stdClass();
            $data->id = $conflicts[$key]->id;
            $data->idstudents= $students;
            $DB->update_record('peerforum_peergrade_conflict', $data);
        }

        foreach ($can_be_assigned as $k => $value) {
            if(in_array($can_be_assigned[$k], $conflictstds) && in_array($postauthor, $conflictstds)){
                $i = array_search($can_be_assigned[$k], $can_be_assigned);
...
function peerforum_print_post($post, $discussion, $peerforum, &$cm, $course, $ownpost=false, $reply=false, $link=false,
                          $footer="", $highlight="", $postisread=null, $dummyifcantsee=true, $istracked=null, $return=false, $peergrade=true, $showincontext=false, $to_peergrade_block=true, $url_block=null) {
   ...
   if(!$allowpeergrade){
    ...
    if (empty($str)) {
        ...
        $str->rating        = get_string('rating', 'peerforum');
    ...
    // Zoom in to the parent specifically
     ...
     if($showincontext){
            ...
            // $post->peergrade->onblock = '1';
             //$post->peergrade->topic = $post->subject;
     }
...
///////
    ...
    if($showincontext){
    $output .= html_writer::tag('div', 'PeerForum: '.$peerforum->name, array('class'=>'title',
                                                           'role' => 'heading',
                                                           'aria-level' => '1',
                                                            'style' => 'color: #7F7F81'));

    $output .= html_writer::tag('div', 'Subject: '.$postsubject, array('class'=>'subject',
                                                           'role' => 'heading',
                                                           'aria-level' => '2',
                                                           'style' => 'color: #7F7F81'));
...
function peerforum_rating_validate($params) {
    ...
    $post = $DB->get_record('peerforum_posts', array('id' => $params['itemid'], 'userid' => $params['rateduserid']), '*', MUST_EXIST);
    ...
    if ($params['rating'] < 0  && $params['rating'] != RATING_UNSET_RATING) {
        throw new rating_exception('invalidnum1');
    }
    ...
    if ($peerforum->scale < 0) {
        ...
        if ($scalerecord) {
            ...
            if ($params['rating'] > count($scalearray)) {
                throw new rating_exception('invalidnum2');
            }
        } else {
            throw new rating_exception('invalidscaleid');
        }
        ...
        throw new rating_exception('invalidnum3');
    }
...
function peerforum_add_discussion($discussion, $mform=null, $unused=null, $userid=null) {
    ...
    $discussion->peergradeassessed     = 0;
...
function peerforum_post_subscription($fromform, $peerforum, $discussion) {
    ...
    if (\mod_peerforum\subscriptions::is_forcesubscribed($peerforum)) {
        ...
        if ($subscribed && !has_capability('moodle/course:manageactivities', context_course::instance($peerforum->course), $USER->id)) {
...
function peerforum_print_latest_discussions($course, $peerforum, $maxdiscussions = -1, $displayformat = 'plain', $sort = '',
                                        $currentgroup = -1, $groupmode = -1, $page = -1, $perpage = 100, $cm = null, $to_peergrade_block=true, $url_block=null) {
    global $CFG, $USER, $OUTPUT;
 ...
 * @param bool $canrate
 */
function peerforum_print_discussion($course, $cm, $peerforum, $discussion, $post, $mode, $canreply=NULL, $canrate=false, $cangrade=false, $showincontext=false, $to_peergrade_block=true, $url_block=null, $index=null) {
    ...
    if ($peerforum->assessed != RATING_AGGREGATE_NONE) {
        ...
        $rm = new rating_manager();
        $posts2 = $rm->get_ratings($ratingoptions);
    }
...
function peerforum_reset_userdata($data) {
    ...
    if( $removeposts || !empty($data->reset_peerforum_ratings) ) {
        ...
        $peergradedeloptions->ratingarea = 'post';
    }
    ...
    if ($removeposts) {
        ...
        if ($peerforums) {
            foreach ($peerforums as $peerforumid=>$unused) {
                ...
                $pm->delete_peergrades($peergradedeloptions);
            }
        }
    ...
    if (!empty($data->reset_peerforum_ratings)) {
        if ($peerforums) {
            foreach ($peerforums as $peerforumid=>$unused) {
                ...
                $peergradedeloptions->contextid = $context->id;
                $pm->delete_peergrades($ratingdeloptions);
            }
        }
...
function peerforum_reset_course_form_defaults($course) {
    return array('reset_peerforum_all'=>1, 'reset_peerforum_digests' => 0, 'reset_peerforum_subscriptions'=>0, 'reset_peerforum_track_prefs'=>0, 'reset_peerforum_ratings'=>1);
}
