...
class restore_peerforum_activity_structure_step extends restore_activity_structure_step {
    ...
    protected function define_structure() {
        ...
        if ($userinfo) {
            ...
            $paths[] = new restore_path_element('peerforum_discussion_sub', '/activity/peerforum/discussions/discussion/discussion_subs/discussion_sub');
            $paths[] = new restore_path_element('peerforum_rating', '/activity/peerforum/discussions/discussion/posts/post/ratings/rating');
    ...
    protected function process_peerforum_peergrade($data) {
        ...
        $data->contextid = $this->task->get_contextid();
        $data->itemid    = $this->get_new_parentid('peerforum_post');
        if ($data->peergradescaleid < 0) { // scale found, get mapping
            $data->peergradescaleid = -($this->get_mappingid('peergradescale', abs($data->peergradescaleid)));
        }
    ...
    protected function after_execute() {
        ...
        if ($peerforumrec->type == 'single' && !$DB->record_exists('peerforum_discussions', array('peerforum' => $peerforumid))) {
            ...
            $sd->peerassessed = $peerforumrec->peerassessed;
