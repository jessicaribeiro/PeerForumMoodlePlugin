...
foreach ($posts as $post) {
    ...
    if ($peerforum->assessed != RATING_AGGREGATE_NONE) {
        ...
        $ratingoptions->peergradescaleid = $peerforum->peergradescale;
    ...
    //add the peergrades information to the post
    //Unfortunately seem to have do this individually as posts may be from different peerforums
    if ($peerforum->peergradeassessed != PEERGRADE_AGGREGATE_NONE) {
        ...
        $peergradeoptions->assesstimestart = $peerforum->peergradeassesstimestart;
        $peergradeoptions->assesstimefinish = $peerforum->peergradeassesstimefinish;
        $peergradeoptions->feedback = $peerforum->feedback;
        $postswithpeergrades = $pm->get_peergrades($peergradeoptions);

        if ($postswithpeergrades && count($postswithpeergrades)==1) {
            $post = $postswithpeergrades[0];
        }
    }
    ...
    peerforum_print_post($post, $discussion, $peerforum, $cm, $course, false, false, false,
            $fulllink, '', -99, false, false, false, true, null);
}
