...
public function render_peergrade(peergrade $peergrade) {
        ...
        //$peergrademanager->initialise_assignpeersparent_javascript($PAGE);
        ...
        /*---------------------------------*/
            ...
            //DISPLAY FEEDBACK
                ...
                if(!empty($all_feedback)){
                    ...
                    if(($already_peergraded_by_user && $editpostid == -1) || ($USER->id == $peergrade->itemuserid && $already_peergraded_by_user) || !$isstudent || !$is_assigned || ($is_assigned && $post_expired)){
                        $peergradehtml .= $OUTPUT->action_link($CFG->dirroot . '/mod/peerforum/collapse.php', $expandstr, new component_action('click', 'peerforum_collapse', array('postid' => $peergrade->itemid)), array('id' => 'actionlink'.$peergrade->itemid));
                    }

/*DIV*/             $peergradehtml .= html_writer::start_tag('div', array('id'=> 'peergradefeedbacks'.$peergrade->itemid, 'class' => 'peergradefeedbacks', 'style' => 'display:none;'));
                    ...
                    foreach ($all_feedback as $i => $value){
                        ...
                        if($can_see_grades && $time_see_grades || $can_see_feedbacks && $time_see_feedbacks){
                            ...
                            $postauthor = $DB->get_record('peerforum_posts', array('id' => $peergrade->itemid))->userid;
                ...
                /*DIV*/        $peergradehtml .= html_writer::end_tag('div');
        ...
        if(has_capability('mod/peerforum:viewallpeergrades', $PAGE->context)){
            if(!$isstudent){
                ...
                $peergradehtml .= $OUTPUT->action_link($CFG->dirroot . '/mod/peerforum/collapse.php', $expandstr, new component_action('click', 'peerforum_collapse_config', array('postid' => $peergrade->itemid)), array('id' => 'actionlink_config'.$peergrade->itemid));
/*DIV*/         $peergradehtml .= html_writer::start_tag('div', array('id'=> 'peergradeconfig'.$peergrade->itemid, 'class' => 'peergradeconfig', 'style' => 'display:none;'));
                ...
                $post_id = $peergrade->itemid;
                $postauthor = $DB->get_record('peerforum_posts', array('id' => $post_id))->userid;

                //Assign peer grader
                $PAGE->requires->js('/mod/peerforum/assignpeer.js');

                $assignpeerurl = new moodle_url('/mod/peerforum/assignpeer.php', array('itemid' => $post_id, 'courseid' => $courseid, 'postauthor' => $postauthor));
                $formattrs = array(
                'id'     => "menuassignpeerform{$post_id}",
    ...
    /*FORM1*/   $peergradehtml  .= html_writer::start_tag('form', $formattrs);
                ...
                if(empty($peers_topeergrade)){
                    $peergradehtml .= html_writer::tag('span', ' None', array('id' => 'peersassigned'.$post_id, 'style'=> 'color: grey;'));
                    //$assignpeergradersparent = get_string('assignpeergradersparent', 'peerforum');
                    //$peergradehtml .= $OUTPUT->action_link($CFG->dirroot . '/mod/peerforum/assignstudent.php', $assignpeergradersparent, new component_action('click', 'peerforum_collapse', array('postid' => $peergrade->itemid)), array('id' => 'actionlink'.$peergrade->itemid));

                }

                if(empty($peers_topeergrade)){
/*DIV*/             $peergradehtml .= html_writer::start_tag('div', array('id'=> 'nonepeers'.$peergrade->itemid, 'class' => 'nonepeers', 'style' => 'display:block;'));
                } else {
/*DIV*/             $peergradehtml .= html_writer::start_tag('div', array('id'=> 'nonepeers'.$peergrade->itemid, 'class' => 'nonepeers', 'style' => 'display:none;'));
...
//aqui
                ...
                $peergradehtml .= $OUTPUT->action_link($CFG->dirroot . '/mod/peerforum/assignpeersparent.php', $assignpeersparentstr, new component_action('click', 'peerforum_assignpeersparent', array('itemid' => $peergrade->itemid, 'courseid' => $courseid, 'postauthor' => $postauthor)), array('id' => 'actionlinkpeers'.$peergrade->itemid));
                ...
                if(!empty($peers_topeergrade)){
                    ...
                    foreach ($peers_topeergrade as $key => $value) {
                        ...
                        if($key != $last_key){
                            array_push($peernames, html_writer::tag('span', $peer_name, array('id' => 'peersassigned'.$post_id, 'style'=> 'color:'.$color.';')));
                    ...
                    $peergradehtml .= html_writer::tag('span', $peersnames, array('id' => 'peersassigned'.$post_id, 'style'=> 'color:'.$color.';'));

                }

                $selectstudentrandom = get_string('selectstudentrandom', 'peerforum');
                //$selectstudentstr = get_string('selectstudent', 'peerforum');
            ...
            //    if(!empty($students_assign)){
                    ...
                    $studentattrs = array('class'=>'menuassignpeer studentinput','id'=>'menuassignpeer'.$post_id);
                    $peergradehtml .= html_writer::select($studentsarray, 'menuassignpeer'.$post_id, $studentsarray[UNSET_STUDENT_SELECT], false, $studentattrs);

            //    } else {
            //        $studentattrs = array('class'=>'menuassignpeer studentinput','id'=>'menuassignpeer'.$peergrade->itemid, 'disabled' => true);
            //        $peergradehtml .= html_writer::select($studentsarray, 'menuassignpeer'.$peergrade->itemid, $studentsarray[UNSET_STUDENT_SELECT], false, $studentattrs);
            //    }
    ...
    /*FORM1*/   $peergradehtml  .= html_writer::start_tag('form', $formattrs_rmv);
                ...
                //Remove peer grader
                $PAGE->requires->js('/mod/peerforum/removepeer.js');

                $removepeerurl = new moodle_url('/mod/peerforum/removepeer.php', array('itemid' => $post_id, 'courseid' => $courseid, 'postauthor' => $postauthor));
                $formattrs = array(
                'id'     => "menuremovepeerform{$post_id}",
                'class'  => 'menuremovepeerform',
                'method' => 'post',
                'action' => $removepeerurl->out_omit_querystring()
                );

    /*FORM3*/   $peergradehtml .= html_writer::start_tag('form', $formattrs);
                ...
                //if(!empty($students_assigned_rmv)){
                    ...
                    $studentattrs_rmv = array('class'=>'menuremovepeer studentinput','id'=>'menuremovepeer'.$post_id);
                    $peergradehtml .= html_writer::select($studentsarray_rmv, 'menuremovepeer'.$post_id, $studentsarray_rmv[UNSET_STUDENT], false, $studentattrs_rmv);

                //} else {
                //    $studentattrs_rmv = array('class'=>'menuremovepeer studentinput','id'=>'menuremovepeer'.$post_id, 'disabled' => true);
                //    $peergradehtml .= html_writer::select($studentsarray_rmv, 'menuremovepeer'.$peergrade->itemid, $studentsarray_rmv[UNSET_STUDENT], false, $studentattrs_rmv);
...
/*FORM3*/       $peergradehtml .= html_writer::end_tag('form');
/*FORM1*/       $peergradehtml .= html_writer::end_tag('form');
