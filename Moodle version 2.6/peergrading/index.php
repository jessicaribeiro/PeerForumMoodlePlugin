...
if($display == '3'){
    if(has_capability('mod/peerforum:viewpanelpeergrades', $context)){
        ...
        if(!isset($CFG->seeoutliers)){
            $seeoutliers = 1;
        } else {
            $seeoutliers = $CFG->seeoutliers;
        }

        if(!isset($CFG->outlierdetection)){
            $outlierdetection = 'standard deviation';
        } else {
            $outlierdetection = $CFG->outlierdetection;
        }

        if(!isset($CFG->outdetectvalue)){
            $outdetectvalue = 1;
        } else {
            $outdetectvalue = $CFG->outdetectvalue;
        }
            ...
            foreach ($info as $postid => $value) {
                ...
                for($i = 0; $i < $count_peergrades; $i++){
                    ...
                    if($grade >= $min_interval && $grade <= $max_interval){
                        ...
                        //$color_grade = '<font color="black">'.$grade.'</font>';
                    } else {
                        //outlier
                        $color_grade = '<font color="red">'.$grade.'</font>';
                    }

                    echo '<tr>'.'<td bgcolor='."$color".'>'. $grader_link .'</td>'.'<td bgcolor='."$color".'>'.$color_grade.'</td>'.'<td bgcolor='."$color".'>'.$feedback.'</td>'.'</tr>';
                }
