...
function  get_posts_expired_infograder($infograder, $courseid){
    ...
    foreach ($infograder as $id => $value) {
        ...
        $postsexpired = $DB->get_record('peerforum_peergrade_users', array('courseid' => $courseid, 'iduser' => $authorid))->postsexpired;
        $posts = array();
        if(!empty($postsexpired)){
            $postsexpired = explode(';', $postsexpired);
            $postsexpired = array_filter($postsexpired);
...
function standart_deviation($array) {
    ...
    if($div != 0){
        ...
        return sqrt(array_sum(array_map("sd_square", $array, array_fill(0, count($array), (array_sum($array) / count($array))))) / $div);
    }
}
...
function can_user_peergrade($already_peergraded_by_user, $editpostid, $post_topeergrade){
    if((!$already_peergraded_by_user && $editpostid == -2) || ($already_peergraded_by_user && $editpostid == -2) || (!$already_peergraded_by_user && $editpostid == -1) || ($already_peergraded_by_user && $editpostid == $post_topeergrade)){
        return true;
    } else {
        return false;
    }
...
function get_peersnames($peers_topeergrade, $post_topeergrade){
    ...
    foreach ($peers_topeergrade as $key => $value) {
        $graded = $DB->get_record('peerforum_peergrade', array('itemid' => $post_topeergrade, 'userid' => $peers_topeergrade[$key]));
        if(!empty($graded)){
            $color = '#339966';
        } else {
            $color = '#cc3300';
        ...
        if($key != $last_key){
            array_push($peernames, html_writer::tag('span', $peer_name, array('id' => 'peersassigned'.$post_topeergrade, 'style'=> 'color:'.$color.';')));
...
function peerforum_print_post($post, $discussion, $peerforum, &$cm, $course, $ownpost=false, $reply=false, $link=false,
    ...
    if ($reply && !$showincontext) {
        $commands[] = array('url'=>new moodle_url('/mod/peerforum/post.php#mformpeerforum', array('reply'=>$post->id, 'page' => $currentpage)), 'text'=>$str->reply);
    }
 ...
 *            rateduserid => int the id of the user whose items have been rated. NOT the user who submitted the ratingpeers. 0 to update all. [required]
 ...
 */
 function peerforum_print_discussion($course, $cm, $peerforum, $discussion, $post, $mode, $canreply=NULL, $canratepeer=false, $cangrade=false, $showincontext=false, $to_peergrade_block=true, $url_block=null, $index=null, $start=null, $perpage=null, $enable_pagination=null) {
     global $USER, $CFG;
     ...
     if(!isset($index)){
         switch ($mode) {
             ...
             case PEERFORUM_MODE_NESTED :
                 peerforum_print_posts_nested($course, $cm, $peerforum, $discussion, $post, $reply, $peerforumtracked, $posts, $showincontext, $to_peergrade_block, $url_block, $actual_page);
                 break;
         }
     }
 }
 ...
 * @return void
 */
 function peerforum_print_posts_nested($course, &$cm, $peerforum, $discussion, $parent, $reply, $peerforumtracked, $posts, $showincontext=false, $to_peergrade_block=true, $url_block=null, $actual_page=null) {
    global $USER, $CFG;
    ...
    if (!empty($posts[$parent->id]->children)) {
        ...
        foreach ($posts as $post) {
            ...
            peerforum_print_post($post, $discussion, $peerforum, $cm, $course, $ownpost, $reply, $link,
                                 '', '', $postread, true, $peerforumtracked, false, true, $showincontext, $to_peergrade_block, $url_block, $actual_page);
            peerforum_print_posts_nested($course, $cm, $peerforum, $discussion, $post, $reply, $peerforumtracked, $posts, $showincontext, $to_peergrade_block, $url_block, $actual_page);
            echo "</div>\n";
        }
    }
